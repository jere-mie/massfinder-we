...
Compares bulletin PDFs to existing church data loaded from SQLite and returns markdown.
...
def update_churches_from_markdown(churches_data, markdown_report, markdown_results, model=None):
    """
    Use LLM to analyze the markdown report and return updated church records.
    
    Args:
        churches_data: List of church dictionaries loaded from the SQLite database
    ...
    try:
        logger.info("Sending current church records and analysis report to LLM for updates...")
...
CURRENT CHURCH RECORDS:
{json.dumps(churches_data, indent=2)}

ANALYSIS REPORT (differences found between bulletins and database):
{markdown_report}

TASK: Review the analysis report and update the church records to correct any discrepancies found in the bulletins.
...
OUTPUT: Return ONLY the complete updated church record array as valid JSON. No markdown, no explanations."""
